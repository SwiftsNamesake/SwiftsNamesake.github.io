...
var Context3D = function(canvas) {
	...
	this.setMatrixUniforms = function(values) {
		...
		// console.log('Light at: ', values.light);
		this.context.uniform1i(this.program.uniforms['mode'], false, 1);                 //
	...
	this.loadTexture = function(path) {
		...
		image.onerror = function() { console.assert(false, 'Failed to load image'); promise.reject(image); };
		image.src = path;
